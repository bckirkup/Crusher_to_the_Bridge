...
def _uv_build_finding(path: Path, line: int, tokens: list[str]) -> list[Finding]:
    if not _has_flag(tokens, "--no-build"):
        return [
            Finding(
...
def _uv_sync_findings(path: Path, line: int, tokens: list[str]) -> list[Finding]:
    if not _has_flag(tokens, "--locked") and not _has_flag(tokens, "--frozen"):
        return [
            Finding(
                path,
                line,
...
def _uv_install_findings(path: Path, line: int, tokens: list[str]) -> list[Finding]:
    ...
    if from_requirements:
        ...
        if local_install or _has_flag(tokens, "--require-hashes"):
            return []
...
def _pip_findings(path: Path, line: int, tokens: list[str]) -> list[Finding]:
    ...
    findings: list[Finding] = []
    if not _has_flag(install_tokens, "--only-binary") and not local_install:
        findings.append(
            Finding(
                path,
                line,
    ...
    if local_install or _has_flag(install_tokens, "--require-hashes"):
        return findings
