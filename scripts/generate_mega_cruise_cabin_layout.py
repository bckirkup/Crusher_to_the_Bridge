...
# Dining venue fields added to this platform's committed layout by the
# SEAT-01/02 change but never present on the legacy messy hull this file
# derives from -- so they are applied here, keyed by the renamed id.
DINING_FIELD_OVERRIDES = {
    "Pool_Bar_Grill": {"dining_service_type": "buffet",
                       "food_contamination_multiplier": 3.0, "meal_seatings": 2},
    "MainDining_L": {"dining_service_type": "mdr",
                     "food_contamination_multiplier": 1.0, "meal_seatings": 2},
    "MainDining_U": {"dining_service_type": "mdr",
                     "food_contamination_multiplier": 1.0, "meal_seatings": 2},
    "Windjammer": {"dining_service_type": "buffet",
                   "food_contamination_multiplier": 3.0, "meal_seatings": 2},
    "SpecRest_A": {"dining_service_type": "specialty",
                   "food_contamination_multiplier": 0.5, "meal_seatings": 2},
    "SpecRest_B": {"dining_service_type": "specialty",
                   "food_contamination_multiplier": 0.5, "meal_seatings": 2},
    "CafeBakery": {"dining_service_type": "buffet",
                   "food_contamination_multiplier": 3.0, "meal_seatings": 2},
    "Crew_Mess_Main": {"dining_service_type": "crew_mess",
                       "food_contamination_multiplier": 1.0, "meal_seatings": 3},
    "CrewMess_Fwd": {"dining_service_type": "crew_mess",
                     "food_contamination_multiplier": 1.0, "meal_seatings": 3},
    "ComedyClub": {"dining_service_type": "specialty",
                   "food_contamination_multiplier": 0.5, "meal_seatings": 2},
    "Main_Galley_Aft": {"dining_service_type": "galley",
                        "food_contamination_multiplier": 0.5},
    "BuffetGal_U": {"dining_service_type": "galley",
                    "food_contamination_multiplier": 0.5},
    "SpecGalley": {"dining_service_type": "galley",
                   "food_contamination_multiplier": 0.5},
}
...
def _public_zones(legacy: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    zones = [
        deepcopy(z) for z in legacy["zones"]
        if not z["id"].startswith(skip_prefixes)
        # Legacy sanitary blocks are regenerated by provisioning on this
        # hull's own zones, not copied -- their serves lists reference the
        # legacy ids this pass renames.
        and z.get("type") != "Sanitary"
    ]
    for zone in zones:
        zone["id"] = LEGACY_TO_MEGA_ZONE.get(zone["id"], zone["id"])
        zone.update(DINING_FIELD_OVERRIDES.get(zone["id"], {}))
    return zones
...
def build_spatial_layout(legacy: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "description": (
            ...
            "and 12 crew corridor sections (~170 crew each) replace legacy well-mixed "
            "berthing blocks. See docs/PLATFORM_CABIN_REVISION.md; legacy topology "
            "archived as messy_cruise_500."
        ),
        ...
        "nominal_complement": {"passengers": 5000, "crew": 2000},
        "zones": zones,
    }
...
def build_air_flow_paths(legacy_airflow: dict[str, Any], zone_ids: set[str]) -> dict[str, Any]:
    ...
    for hz in legacy_airflow["hvac_zones"]:
        if hz["id"].startswith("AHU_Network_Cabins"):
            continue
        if hz["id"].startswith("AHU_HD_"):
            continue  # regenerated below from this hull's provisioning
        entry = dict(hz)
    ...
    cross_links = [
        ...
        and cl.get("path") != "Sanitary_Makeup"
    ]
    ...
    # Sanitary head blocks: exhaust-only group + one-way makeup link each.
    with open(LEGACY_LAYOUT, encoding="utf-8") as fh:
        mega_zones = build_spatial_layout(json.load(fh))["zones"]
    augmented = sanitary_airflow_augment(mega_zones, {
    ...
    return {
        ...
        "hvac_zones": augmented["hvac_zones"],
        "cross_zone_links": augmented["cross_zone_links"],
        "adjacency": adjacency,
    }
