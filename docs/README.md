...
| [covid/covid_channel_anchor_handoff_2026_09_25.md](covid/covid_channel_anchor_handoff_2026_09_25.md) | Handoff record (2026-09-25) — where the conditional-gap hunt stands after PARTNER-RATE-V1 / PLUME-DOSE-V1 / ROUTE-ATTR-V1 / LAMBDA-CROSS-V1: the sampled ring and plume dose cleared, Θ bound but short, the ~11× channel over-inclusion localised to onset dating, and the single open decision (the proposed SERO-CHANNEL-V1 design — `covid.H5` serology anchor + period onset-recording channel + θ×channel grid). Numbers live in the ledger entries it cites |
| [covid/covid_theta_screen_v10_readout.md](covid/covid_theta_screen_v10_readout.md) | Findings (2026-09-21) — the full v10 surface, 200/200 cells on the post-QUAR-EXEMPT-01/REINFECT-01 engine: takeoff identical to v9 at every decade, zero takeoff-class flips in 200 paired seeds, shared-seed cells of record reproduced exactly; v10 supersedes v9 as surface of record, `covid.T3` flips at 1e9 on the same bimodal span |
...
| [norovirus/high_touch_area_handoff_2026_09_22.md](norovirus/high_touch_area_handoff_2026_09_22.md) | Handoff record (2026-09-22) — where the fomite areal denominator stands: why `HIGH_TOUCH_AREA_M2` is a lumping parameter rather than ship geometry, the three-reading span against the single definition, why the #37 empty verdict is pre-refit, the post-era A4 hole on two of four hulls, and the single open decision (sweep `A` as a ranged coordinate vs retire it by disaggregating). Reports no new numbers; every figure is quoted from the entry that measured it |
