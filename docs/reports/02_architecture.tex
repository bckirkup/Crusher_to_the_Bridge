...
Short-range droplet exposure converts a fraction of shedding to same-room aerosol mass. The implementation uses a droplet aerosolization fraction and an inhalation fraction per epoch. For
target $i$ in zone $z$,
\begin{equation}
D_{i,p,\mathrm{droplet}}(t)=\phi_{\mathrm{drop}}\,\psi_{\mathrm{inh}}\,
\sum_{j\in\mathcal{I}_{p,z}(t)}V_{j,p}(t)\,q_i(t)\,b_z,
\end{equation}
where $\phi_{\mathrm{drop}}$ is the droplet aerosol fraction, $\psi_{\mathrm{inh}}$ is the inhaled fraction, $q_i(t)$ is the confinement factor, and $b_z$ is the balcony or zone ventilation
...
\begin{equation}
D_{i,p,\mathrm{airborne}}(t)=\psi_{\mathrm{inh}}\,\frac{M_{z,p}(t)}{V_z}\,V_z\,a_{\mathrm{HVAC}}(t)b_z,
\end{equation}
where $a_{\mathrm{HVAC}}$ is the active HVAC-airborne scalar injected by protocols. The algebra cancels $V_z$ in this implementation because inhalation is represented as a fraction of room
mass rather than as an explicit breathing-flow term. That simplification is adequate for campaign comparisons but should be refined for absolute inhaled-dose validation.
