...
| [adj_stratified_readout.md](adj_stratified_readout.md) | Measurement (2026-09-11) | The four expedition campaigns re-read within a dose regime, on arrays already on disk: the large contact arms were diluted ~8× by the inert part of the box rather than null, A5 is flat at 2.56–2.75 in **every** regime against ≈3.5, and the import-only stratum still posts 4.5% against A9's 0.42–0.56% — so no transmission-side change reaches the posting anchor. §3 then locates that floor: at `adj` ≥ 10, where the profiled release is arithmetically dead, 2.51% of voyages still post on A9's own passenger channel, because the hand route carries its own unswept normaliser (an implicit −7.14 log10 g of stool per hand). Nothing adopted, no stratum declared |
