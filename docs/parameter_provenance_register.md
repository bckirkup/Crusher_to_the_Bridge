...
| Observation model (~15 numbers) | — | C / F | One empirical aggregate constrains them jointly, and that aggregate is anchor A3 — which is why A3 no longer scores: it is reported against a 0.35–0.45 construction band and carries no verdict (#23 done). The fifteen numbers themselves are still unsourced | — | — | — | L? | ⊘ **A3 is not a test** — now enforced in `score_anchors.py`, not only asserted here | #23 done, #27 |
...
5. **The observation model's ~15 numbers** — jointly constrained by a single
   ...
   declaration is right. Track A3 (#48) changed what those numbers are read
   against and
