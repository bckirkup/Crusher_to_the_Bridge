...
| `shedding_duration_days` | **absent** | — | Tranche 8 added the field and `norwalk_gi` carries 15; this arm does not, so shedding clears at `recovery_day` = 7 and **7 of the 15 authored curve days are unreachable** — 0.021% of the authored curve mass, the tail being 2.0–5.5 log10, against 38.5% for norovirus before the tranche-8 fix. A **detectability** defect rather than a dose defect. Evidence for the tail: Keske 2023 culture positivity 83/52/13.5/8% at days 5/7/10/14, 19% infectious after symptoms stop | Keske 2023: **R** (median culture-positive duration in body chunks) | — | — | L? | ⊘ **field exists, value absent on this arm** — blocks #33, since Diamond Princess is a repeat-testing series over weeks | #51 |
