...
| Emesis titre (engine) | **withdrawn as an input**; module constant 3.9e4 retained only for importers of the withdrawn figure | B, **from the abstract** | Kirby 2016 Table 3: GII.2 = 1.6e5. The abstract pools a 2-subject GII.1 pilot the Results exclude. No profile key resolves to a titre any more; the emesis record carries titre as a **derived** diagnostic (`episode_load / volume_ml`), so the abstract-versus-Results defect cannot return through a config | — superseded by the cumulative total | #38 done |
