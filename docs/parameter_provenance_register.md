...
| `never_symptomatic_fraction` — share of imported infections that never present, in a boarding population | **none shipped** | **n/a (declared axis)** | **Unsearched.** No literature was consulted for this change; the coordinate is declared, not sourced. It is a config-supplied sweep axis of the initiation engine ([spec](proposals/initiation_engine_spec.md) §5), and enabling boarding without it is a **load error** rather than a defaulted run | ⊘ **no value licensed** — sweep it, or source it, before boarding is enabled | #45 |
| `presymptomatic_share_of_presenting` — share of presenting imported infections caught before onset | 0.04 as a derived default, swept | C, **derived, not measured** | Derived arithmetic on this profile's own clocks, not an independent measurement: a presenting episode occupies `presymptomatic_shedding_days` 0.5 before onset and `shedding_duration_days` 15 − `recovery_day` 3 = 12 after clearance of illness, so 0.5 / 12.5 ≈ 0.04 of the presenting prevalent pool is pre-symptomatic ([spec](proposals/initiation_engine_spec.md) §4) | Default shipped in the config block only; swept, never adopted as a measured value | #45 |
...
| `surface_decay_per_day` | 0.95 | C → **B** | **Corroborated under both readings of the field**: as a per-day fraction = 5.55 h half-life, as 1.30 log10/day = 7.6 h, both inside Xu 2023's reviewed 5–9 h on stainless steel / plastic / glass at 22 °C. Material and condition dependence is large and is not carried by a single constant (porous 1–5 h; 28 days at 20 °C in darkness with a protein-rich matrix, Riddell; 90% loss every 6.8–12.8 min under simulated UVB, Ratnesar-Shumate) | — in tree; the **first inherited SARS-CoV-2 constant a search confirms** ([tranche 9](literature/mql_tranche_9_sars_cov2.md) §4) | |
...
| `dose_response.alpha` / `beta` | 0.18 / 58.0 | C (attribution withdrawn twice, independently) | Killingley 2022 human challenge → ID50 ≈ 9.2 TCID50 ≈ 10³–10⁴ copies; Zhang & Wang 2020 → 4.4e4–6.8e5 copies. **The two disagree by 1–2 orders; that disagreement is the interval.** Tranche 9 adds two further candidates and **rejects both**: Prentiss (ID50 361–2,000 particles) and Riediker (500/300/100 copies for WT/Delta/Omicron) are **fitted to attack-rate data**, so adopting either and then scoring on Diamond Princess would fit a physical constant to a scored anchor. Killingley remains the only independent measurement, and it is in TCID50 — the #43 unit trap | ⊘ joint — sweep 10³–7e5, adopt neither endpoint, and **no attack-rate-fitted value** | #30 |
