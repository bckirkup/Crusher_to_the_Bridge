...
**Status:** Merged additive fragment; kept as the audit trail. Proposed replacement rows for
[`../../parameter_provenance_register.md`](../../parameter_provenance_register.md)
§3.3, recorded in the register. Sourcing tranche:
...
**Superseded in the tree.** Both rows below have since been replaced: R3 deleted
the dose-dependent `illness_probability.eta`/`gamma` pair in favour of a
dose-independent `symptomatic_fraction` carrying the same Carrat 66.9%, and
tranche 19 reopened the surface-decay null this fragment recorded. The register
carries the current rows; this remains the audit trail of what was proposed at
tranche 14.
