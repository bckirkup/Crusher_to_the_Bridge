...
>
> **Two figures below were corrected at merge and must not be quoted from here.** (1) The household-SAR analogue reads **14–23%** below, but 23% was uncited — the register carries **13.8–23%** with all three sources (Matsuyama 2018 13.8%, Quee 2020 15%, Balachandran 2023 23%) and with Balachandran's two caveats, that its 23% is pooled viral AGE rather than norovirus-specific and is household-size-conditioned. (2) The empirical bound **`f ≤ 0.18–0.45`** was derived from cruise outbreak attack rates, which are the same class of data anchors A4/A8/A9 score against; it is retained as evidence but is barred from entering a likelihood alongside those anchors, and the **structural bound is the headline bound**. The register's §3.1 row and §3.4 rejection row are authoritative on both.
>
> **A third figure below is superseded.** The structural ceiling is written here as `f ≤ 0.5`, which is the exact-double-occupancy case. Task #12 sourced the berthing plan the identity `f ≤ 1 − cabins/occupants` actually needs — published cruise occupancy is passengers per lower berth, two per cabin by definition — and every published full year since the restart exceeds unity, so the fleet-wide ceiling is **`f ≤ 0.500–0.539`** and 0.5 is its floor. In-tree hulls carry their own ceilings off `default_cabin_size`. Nothing about the null results below changes: `f` is still measured nowhere and `f = 0` is still not excluded.

**Status:** additive fragment, tranche 17 / unit `cabin-localization` / task
**#12**. Merged into the register; kept as the audit trail for one row of
