...
| A3 | Symptom severity gets a trajectory | #48 | Severity is drawn once at onset and held for the whole illness, so what an observer sees does not vary over the course — and observability is the quantity this model exists to predict |
