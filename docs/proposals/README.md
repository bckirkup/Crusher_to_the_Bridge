...
| [initiation_engine_spec.md](initiation_engine_spec.md) | The boarding draw, the `initiation` config block, and the run-artifact provenance — all of it | Four seeding paths exist in-tree (`KorkinShipEngine._seed_initial_infection`, `orchestrator_init.py`, `step_mid_cruise_introductions`, `step_shore_introductions`) and all four are constructions: a deterministic count of 1, no role split, infection age 0, and a hard-coded dose in two different values. The measured boarding prevalence of tranche 10 has nowhere to enter until the block exists. Its §5 records an open decision rather than assuming one, and enabling the block on the COVID arm is blocked on task #51 |
