...
### 7. Clinical Impression (bedside, no lab test)
For pathogens without rapid diagnostic tests, the cascade Tier 1 relies on 
clinical recognition. These numbers represent a trained shipboard clinician's 
ability to correctly suspect the pathogen based on presentation alone.

| Pathogen | Sensitivity (day 1) | Sensitivity (day 3) | Sensitivity (day 5+) | Specificity | TAT | Cost | Notes |
|----------|--------------------|--------------------|---------------------|-------------|-----|------|-------|
| Measles | 0.20 | 0.50 | 0.80 | 0.70 | 0 | 0 | Pre-rash prodrome nonspecific; Koplik spots day 2-3; classic rash day 3-5 raises sens |
| Ebola | 0.15 | 0.30 | 0.60 | 0.40 | 0 | 0 | Early illness indistinguishable from flu/malaria; hemorrhagic signs late (minority); EVD clinical score: 81% sens, 54% spec |
| Hantavirus | 0.10 | 0.25 | 0.70 | 0.50 | 0 | 0 | Prodrome nonspecific; abrupt cardiopulmonary phase day 4-5 is distinctive but late; CBC rule 89% sens, 93% spec but needs lab |
| Campylobacter | 0.15 | 0.20 | 0.25 | 0.30 | 0 | 0 | Clinically indistinguishable from other bacterial GI; bloody diarrhea in ~50% but not pathognomonic |
| Legionella | 0.30 | 0.40 | 0.50 | 0.50 | 0 | 0 | GI symptoms + pneumonia + hyponatremia raise suspicion; water system exposure history on ship is key |
| Vibrio | 0.25 | 0.35 | 0.40 | 0.50 | 0 | 0 | Watery diarrhea after seafood; wound infection after seawater exposure; rice-water stool for cholera |

Notes:
- Specificity is LOW because these presentations overlap heavily with common infections
- Clinical impression is a probability modifier, not a definitive test
- On a ship, the key action from clinical suspicion is ISOLATION + confirmatory test ordering
- Exposure history (outbreak awareness) dramatically improves recognition — during a known 
  outbreak, sensitivity increases ~20-30% for all syndromes
- These numbers are approximate; exact pooled estimates do not exist for most syndromes
