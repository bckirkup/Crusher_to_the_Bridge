...
- **Cabin air mode:** `transmission.cabin_air_mode` (`cabin_compartment` default: the short-range inhalation route runs on staterooms, diluting into each cabin's berth share of the declared block volume — `docs/cabin_air_compartment_spec.md`, `AERO-CABIN-01`; `zone_pool` is the labelled pre-change whole-corridor pool kept for paired attribution). Norovirus is a null by construction (zero continuous emission share).
- **Near-field air:** `transmission.near_field_air` defaults to `mode: two_box`, β `interzonal_airflow_m3_per_hour: 204` (Keil 2017 GM, Grade B) and ρ `neighbour_table_ratio: 0.43` (Li 2021, Grade B); `mode: off` is the labelled baseline, and buffet/crew-mess diners receive per-meal table deals.
- **Pathogen pool transport:** `hvac.pathogen_pool_transport` (`airflow` default: every pathogen's own airborne pool moves through the declared airflow network via the exact linear CONTAM operator; `none` is the labelled pre-change baseline in which only the legacy aggregate array was transported and no pathogen's pool ever crossed a zone — `AERO-CABIN-03`, ledger item 46).
...
- **Droplet emission mode:** default `transmission.droplet_emission_mode: profile_conditioned` — an `emesis_conditioned` arm has **no** continuous droplet emission; `shipped_uniform` restores the pre-deletion uniform `DROPLET_AEROSOL_FRACTION` on every arm as a selectable baseline (`docs/density_contact_spec.md`, ledger item 40).
- **Droplet field split:** default `transmission.droplet_field_split.mode: partition` (`AERO-SPLIT-01`, `docs/droplet_field_split_spec.md`) — the zone pool carries only `far_field_share` (0.175, midpoint of declared [0.05, 0.30], Grade C) of continuous droplet emission; the rest reaches only a partner-bounded proximity ring at plume concentration. `mode: off` is the labelled pre-change whole-pool baseline and draws nothing extra on `engine.rng`.
- **Surface swab source:** default `observation.surface_swab_source: surface_pool_density` — the repaired channel: the real deposited `surface_pools_by_pathogen` as a per-cm² density over the zone's high-touch area, with a Park 2015 per-swab copy LOD and `censored_below_lod` flagging (`docs/norovirus/environmental_observation_v1.md`). `airborne_fraction` is the labelled pre-change baseline (the legacy synthetic `microflora.surface_fraction` of the airborne zone pool).
- **Emesis source term:** the engine draws a per-illness host titre (`emesis_titre_gec_per_ml_range`, censored arm `emesis_censored_titre_gec_per_ml_range` for K < 3 episodes) times per-episode volume — `emesis_total_shed_gec_range` is retired, no fallback. Deposition records carry `censored_below_lod`; ledger item 13 reopened (`docs/norovirus/emesis_source_term_v1.md`). The touchable share of a bolus is the high-touch areal fraction of the room and is filed as an `EmesisPatch` localised to the bolus footprint (exposed set = `footprint_area / floor_area` of the zone's occupants), not the zone-wide surface pool — ledger `EMESIS-FOOTPRINT-01`.
- **Blackwater holding tank:** `transmission.blackwater_plumbing` (default `true`; `false` is the labelled pre-change baseline) builds a CSTR raw-influent pool capturing the non-aerosolised bowl share and emesis `non_touchable`; `observation.wastewater_assay_mode: holding_tank` (default; `none` is the labelled baseline) reads it post-discharge with a copies/L qPCR assay (composite Grade C LOD over Alex-Sanders 2023's eluate LOD, Grade A for the assay step; `docs/norovirus/environmental_observation_v1.md` §3). Additive — the graywater sequencing grid is untouched.
- **Norovirus fit status:** `docs/norovirus/norovirus_open_ledger.md` is the live record of what is currently withdrawn — **every dose figure is void pending a refit**. `docs/norovirus/norovirus_model_history.md` is the permanent defect and revision record. Read the ledger before quoting any dose, route share, or anchor result. New entries are one file each under `docs/ledger/<ID>.md` (see its README) with Pathogens and Commit SHA; do not append numbered items.
...
- **Multi-pathogen calibration knobs:** `route_efficiency_multipliers` (legacy `transmission_route_weights`; independent per-route dose multipliers, **not** shares summing to 1), log10 `environmental_faecal_release_log10_g_per_epoch` (legacy `dose_adjustment`), `secretor_negative_fraction` with `secretor_negative_relative_susceptibility` (legacy `innate_nonsusceptible_fraction`, which is sterile immunity in a removed fraction and is refuted for GII), continuous-arm `airborne_emission_fraction` (legacy `surface_deposition_fraction`; it feeds the airborne zone reservoir, never a surface pool), and norovirus's measured per-event `emesis_aerosol_fraction_range` under `airborne_emission_mode = emesis_conditioned`, `agent_behavior` dining/free rotation (default off), Dining `food_contamination_multiplier` / `dining_service_type`, env `source_zones` — see `docs/multi_pathogen_model_changes_spec.md`.
