...
- **Cabin air mode:** `transmission.cabin_air_mode` (`zone_pool` default = the pre-change whole-corridor droplet pool; `cabin_compartment` runs the short-range inhalation route on staterooms, diluting into each cabin's berth share of the declared block volume — `docs/cabin_air_compartment_spec.md`, `AERO-CABIN-01`). Off by default and measured before any default-on; norovirus is a null by construction (zero continuous emission share).
