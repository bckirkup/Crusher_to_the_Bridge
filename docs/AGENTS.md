...
- **Surface swab source:** default `observation.surface_swab_source: airborne_fraction` — the legacy synthetic `microflora.surface_fraction` of the airborne zone pool. `surface_pool_density` selects the repaired channel: the real deposited `surface_pools_by_pathogen` as a per-cm² density over the zone's high-touch area, with a Park 2015 per-swab copy LOD and `censored_below_lod` flagging (`docs/norovirus/environmental_observation_v1.md`).
...
- **Blackwater holding tank:** `transmission.blackwater_plumbing` (default `true`; `false` is the labelled pre-change baseline) builds a CSTR raw-influent pool capturing the non-aerosolised bowl share and emesis `non_touchable`; `observation.wastewater_assay_mode: holding_tank` (default `none`, opt-in) reads it post-discharge with a copies/L qPCR assay (composite Grade C LOD over Alex-Sanders 2023's eluate LOD, Grade A for the assay step; `docs/norovirus/environmental_observation_v1.md` §3). Additive — the graywater sequencing grid is untouched.
