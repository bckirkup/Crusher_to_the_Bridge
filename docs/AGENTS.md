...
- **Blackwater holding tank:** `transmission.blackwater_plumbing` (default `false`) builds a CSTR raw-influent pool capturing the non-aerosolised bowl share and emesis `non_touchable`; `observation.wastewater_assay_mode: holding_tank` (default `none`) reads it post-discharge with a copies/L qPCR assay (composite Grade C LOD over Alex-Sanders 2023's eluate LOD, Grade A for the assay step; `docs/norovirus/environmental_observation_v1.md` §3). Additive and default-off — the graywater sequencing grid is untouched.
