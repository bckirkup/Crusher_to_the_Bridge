...
| `bounded_design_entrypoint.py` | One array child of a bounded design (`--design screen\|region`). Shard index from `AWS_BATCH_JOB_ARRAY_INDEX`; skips work whose artifact is already in S3, and a gate shard resumes its own point stream from S3 after a Spot reclaim. With `--seed-shards > 1` a gate child streams retained per-voyage rows and scores nothing — cells are scored by the merge from pooled blocks. `--only-points "7,13,204"` (one Batch parameter, comma-separated, refused if it is not integer indices) restricts the child to those indices of the same Sobol' grid. | — |
...
| `submit_bounded_design.sh` | Submit one bounded design as a Spot array of `<shard-count>` children (`screen` or `region`); design size via `TRAJECTORIES` / `SOBOL_M` / `SEEDS` / `DESIGN_SEED`. Size the array by **wall-time**, not by trajectory count: `SEED_SHARDS` splits each trajectory's seed set into blocks, so a screen's shard count is `trajectory_shards x SEED_SHARDS` and may run to the queue's 256-vCPU ceiling (20 trajectories x 30 seeds at `SEED_SHARDS=10` is 200 shards of ~33 runs, ~15 min, instead of 20 of ~330). Shards pool only through the design's own merge step, which refuses a trajectory whose seed blocks are not all present. The same applies to the gate, where `SEED_SHARDS` splits a point's matched seed set: the array is `selected points x SEED_SHARDS`, the children upload rows, and the merge scores each pooled cell — which is how a posting frequency near 10^-3 becomes resolvable, since A9's resolution is `1/seeds`. `ONLY_POINTS="7,13,204"` re-runs named grid indices at their own coordinates. Parameters are submitted as JSON, not `key=value` shorthand, because a point selection contains commas. | — |
