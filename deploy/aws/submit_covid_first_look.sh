#!/usr/bin/env bash
# Submit one phase of the replicated COVID first look as an AWS Batch EC2
# Spot array.
#
...
DESIGN="${DESIGN:-covid_first_look_v2}"
...
cells = [c for c in enumerate_cells(design) if c.phase == '$PHASE']
...
PARAMETERS=$(printf '{"phase":"%s","stride":"%s","s3_prefix":"%s","design":"%s"}' \
  "$PHASE" "$STRIDE" "$S3_PREFIX" "$DESIGN_REL")
