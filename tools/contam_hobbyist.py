...
def _safe_repo_path(path: str) -> str:
    """Resolve *path* and require it stay under the repository root."""
...
def load_hobbyist_pack(pack_dir: str | None = None) -> dict[str, Any]:
    ...
    root = _safe_repo_path(pack_dir or DEFAULT_PACK_DIR)
    if not is_path_under_base(REPO_ROOT, root):
        raise ValueError(f"pack_dir escapes repository root: {pack_dir!r}")
    if not os.path.isdir(root):
        raise FileNotFoundError(f"Hobbyist pack directory not found: {root}")
...
def load_hobbyist_overrides(platform_dir: str) -> dict[str, Any]:
    ...
    platform_dir = _safe_repo_path(platform_dir)
    path = os.path.join(platform_dir, "contam", OVERRIDES_NAME)
    if not is_path_under_base(REPO_ROOT, path):
        raise ValueError(f"overrides path escapes repository root: {path!r}")
