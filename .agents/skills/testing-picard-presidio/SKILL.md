...
| Workflow | When |
|----------|------|
| `.github/workflows/ci.yml` | All `main` PRs — full pytest + Presidio smoke + import hygiene |
| `.github/workflows/picard-presidio.yml` | `main` and `cursor/**` — framework slice + Stackelberg schema checks |
