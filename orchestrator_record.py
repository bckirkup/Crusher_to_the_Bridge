...
def _summary_counts(
    ...
    for agent in agents:
        ...
        group = _role_group_for_agent(agent)
        if aid in state.ever_reported_ids:
            reported_counts[group] += 1
        if aid in state.ever_ill_ids:
            ill_counts[group] += 1
    reported_rates = compute_group_rates_for_ids(agents, state.ever_reported_ids)
    ever_ill_rates = compute_group_rates_for_ids(agents, state.ever_ill_ids)
    disrupted_count = sum(
